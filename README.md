...
- 96 孔使用八道排枪直接上样：每组 8 个样本按 A–H 纵向排列，每个样本的复孔向右连续，完成 8 行后再进入右侧下一复孔列块。切换到 384 孔时默认采用推荐的 9 mm 八道隔行路径：先按 A/C/E/G/I/K/M/O 从左到右铺满全部复孔列块，再回到第 1 列按 B/D/F/H/J/L/N/P 铺设第二轮；连续孔位模式仍可选。
...
- “复制含表头”悬停提示对应 QS1/QS3/QS5/QS6/QS7/QSDX/ViiA7；“复制无表头”提示对应 7300 Plus/7500/7500 Fast/StepOnePlus（V2.X 及以上）。
- 首次使用保持空白，可按需载入示例；样本区支持批量清空。
- 所有实验数据仅在当前浏览器本地处理，并在输入后自动保存到 `localStorage`；顶部“保存”按钮可立即保存，刷新或下次打开时自动恢复。
